...
friendsController.getFriends = function(req, res) {
    if (req.query && req.query.friend) {
        ...
        User.findById(req.locals.decoded.id, 'friends').populate('friends.friend_id', 'username')
        .then(user => {
...
friendsController.addFriend = function(req, res) {
    ...
    .then(user => {
        ...
        } else if (req.params.id === req.locals.decoded.id) {
    ...
    .then(user => {
        ...
        var newFriend = {
            friend_id: req.locals.decoded.id
        };
    ...
    .then(() => {
        return User.findById(req.locals.decoded.id);
    })
    .then(user => {
        var newFriendSender = {
...
friendsController.removeFriend = function(req, res) {
    User.update({ _id: req.locals.decoded.id}, { $pull: { 'friends': { friend_id: req.params.id }}}, { safe: true, multi: false })
    .then(() => {
        return User.update({ _id: req.params.id }, { $pull: { 'friends': { friend_id: req.locals.decoded.id }}}, { safe: true, multi: false });
    })
    .then(() => {
        return res.status(200).json({
...
friendsController.acceptFriend = function(req, res) {
    ...
    .then(friend => {
        var myIndex = friend.friends.findIndex(friend => friend.friend_id.toString() === req.locals.decoded.id);
